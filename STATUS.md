...
- Route-run history/action planning is tracked in `docs/dogops/ROUTE_RUN_HISTORY_IMPLEMENTATION_PLAN.md`. Initial implementation now creates global local SQLite route-run history across DogOps runs, mirrors per-run JSON/JSONL exports, creates one historical route run per `Run Route` execution, supports waypoint actions in authored routes, records QR/AprilTag/demo placeholder-image action events, and adds dashboard route-run history/current timeline endpoints and tables. Mission-YAML action defaults, real Go2 image capture, and full incident/work-order timeline ingestion remain planned follow-ups.
