...
- Future simulation fix: `Map From Scratch` animates the DogOps/Rerun replay map, but it is not yet driven by the simulated dog mapping the MuJoCo scene geometry. Real-dog validation should use the live DimOS Go2 map stack; MuJoCo-aligned simulated mapping remains follow-up work.
