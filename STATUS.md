...
- Gemini image comparison is planned in `docs/dogops/GEMINI_IMAGE_COMPARISON_IMPLEMENTATION_PLAN.md`: add optional `gemini_inspect_image` route actions that analyze captured route-run image evidence, compare against the latest previous image for the same waypoint first, store structured analysis as linked evidence, and keep all cloud/API-key behavior optional so the base demo still runs without secrets or internet.
