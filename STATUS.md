...
- Native MuJoCo simulation path: set `DOGOPS_RERUN_VIEW_MODE=native-3d` when the dashboard connects to a full DimOS Go2 simulation Rerun stream. In this mode, `Map From Scratch` publishes `/explore_cmd`, connects 3D View, and refreshes `/api/map` instead of issuing the synthetic DogOps mapping replay.
