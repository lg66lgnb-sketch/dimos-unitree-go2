...
class DogOpsRouteExecutor:
    ...
    def _resolve_route(self, route_id: str | None) -> EditableRoute:
        authoring = load_map_authoring(self.run_dir)
        resolved_id = route_id or authoring.selected_route_id
        if not resolved_id and authoring.routes:
            resolved_id = authoring.routes[0].id
        if not resolved_id:
    ...
    def _wait_until_reached(
        ...
        while self.time_fn() - waypoint_started <= self.waypoint_timeout_s:
            ...
            if odom is None:
                ...
                best_error_m = error_m if best_error_m is None else min(best_error_m, error_m)
                note = f"odom error {error_m:.2f}m"
                if error_m <= self.reach_radius_m:
    ...
    def _append_nav_events(self, route_state: RouteExecutionState) -> None:
        ...
        existing_notes = {event.note for event in state.nav_events}
